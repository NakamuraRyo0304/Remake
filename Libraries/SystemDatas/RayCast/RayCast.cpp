...
{
	auto _outputSize = DX::DeviceResources::GetInstance()->GetOutputSize();
	m_screenSize = { static_cast<float>(_outputSize.right),static_cast<float>(_outputSize.bottom) };
}
...
SimpleMath::Vector3 RayCast::ShotRay()
{
	...
	// 最近距離をスクリーンからワールドに変換
	_nearPos = ConvertScreenToWorld(_mouse.x, _mouse.y, 0.0f,
		static_cast<int>(m_screenSize.x), static_cast<int>(m_screenSize.y),
		m_view, m_projection);

	// 最遠距離をスクリーンからワールドに変換
	_farPos = ConvertScreenToWorld(_mouse.x, _mouse.y, 1.0f,
		static_cast<int>(m_screenSize.x), static_cast<int>(m_screenSize.y),
		m_view, m_projection);
